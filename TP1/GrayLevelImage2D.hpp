...
class GrayLevelImage2D 
{
    ...
    */
    ...
    bool filtrageMedian(int tailleVoisinage);

    bool affiner(int coefficient);

    private:
    // Calcule l'indice dans m_data du pixel (x,y).
    int index(int x, int y) const;
    Container m_data; // Le tableau contenant les valeurs des pixels.
    int       m_width; // la largeur
    int       m_height; // la hauteur
};
